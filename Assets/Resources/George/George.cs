...
// ----------------------ONTRIGGER AND ONCOLLISION FUNCTIONS------------------------
    ...
    protected override void OnTriggerEnter(Collider collision)
    {
        ...
        //if the object collides with another player, employ the lasers and set rewards positively
    ...
    protected override void OnCollisionEnter(Collision collision) 
    {
        ...
        //target collides with the wall, assign negative reward
    ...
    //TODO-2: Add goToBase as an argument to this function ^
    {
        ...
        //if number of balls is >= 3 then go back to base
        if(GetCarrying() >= 3) {
            GoToBase();
        }

        //if number of balls is 0, then go to the farthest target
        if(GetCarrying() == 0) {
            GoToFarthestTarget();
        }

        // from the farthest target, go the nearest one to current position
        if((GetCarrying() >= 1) && (GetCarrying() < 3)) {
            GoToNearestTarget();
        }
    ...
    //Go to the farthest target 
    private void GoToFarthestTarget() {
        GameObject target1 = GetFarthestTarget();
        if (target1 != null){
            float rotation = GetYAngle(target1);
            TurnAndGo(rotation);
        }       
    }
    ...
    // return reference to nearest target 
    protected GameObject GetFarthestTarget(){
        float distance = 200;
